...
export class BookshelfDialogComponent {
  constructor(
  ...
  deleteBook(id_book: string): void {
    if (id_book) {
      this.bookshelfService
        .deleteBookFromBookshelf(id_book, this.data.bookshelf._id)
        .subscribe(
          () => {
            this.snackBar.open('Book successfully deleted', 'OK', {
              duration: 2000,
            });
            this.data.bookshelf.books.splice(
              this.data.bookshelf.books.findIndex(function (i) {
          ...
          () => {
            this.snackBar.open('Book could not be deleted', 'OK', {
              duration: 4000,
            });
          }
        );
