...
module.exports.getLibrary = (req, res) => {
    ...
    if (!ObjectId.isValid(id))
        return res.status(404).json({status: false, message: 'There is no library with given id'});
    else {
...
module.exports.deleteLibrary = (req, res) => {
    const id = req.params.id
    if (!ObjectId.isValid(id))
        return res.status(400).json({status: false, message: 'There is no library with given id'});
    else {
        Library.findOneAndRemove({_id: id}, function (error, result) {
            if (error) {
                res.status(500).send(error);
            } else {
                res.status(200).json({status: true, message: 'Library deleted succesfully'});
            }
        })
    }
}
